...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("sorteocreate")
    .setDescription("Creare un sistema de sorteo")
    .addStringOption((option) =>
      option
        .setName("premio")
        .setDescription("Elije un premio para el sorteo")
        .setRequired(true)
    )
    .addStringOption((option) =>
      option
        .setName("descripcion")
        .setDescription("Elije una descripcion para el sorteo")
        .setRequired(true)
    )
    .addStringOption((option) =>
      option
        .setName("tiempo")
        .setDescription("Elije un tiempo para el sorteo")
        .addChoices(
    ...
    .addChannelOption((option) =>
      option
        .setName("canal")
        .setDescription("Elije un canal para enviar el sorteo")
        .setRequired(true)
    )
    .addRoleOption((option) =>
      option
        .setName("rol")
        .setDescription("Elije un rol para mencionar")
        .setRequired(true)
    ),
  ...
  async execute(interaction) {
    if (
      !interaction.member.permissions.has(
        Discord.PermissionFlagsBits.Administrador
      )
    ) {
      interaction.reply({
  ...
  Clickea el boton para participar.\n**Buena Suerte!!!**`
      ...
      await interaction.reply({
        content: `El sorteo se envio al canal: <#${canal.id}>`,
        ephemeral: true,
      });
      ...
      coletor.on("collect", (i) => {
        if (i.customId === "botao") {
          if (click.includes(i.user.id))
            return i.reply({
              content: `Ya estas participando en el sorteo.`,
              ephemeral: true,
            });
      ...
      setTimeout(() => {
        ...
        interaction.followUp(
          `**Felicitaciones <@${ganhador}> has ganado: \`${premio}\`.**`
        );
      }, duracao);
    }
  },
